...
function ResetPassword() {
	...
	const handleSubmit = (e) => {
		...
		resetPassword({
			...
			.catch((error) => {
				[ email, password, passwordConfirmation ].forEach(({ parseServerError }) =>
			...
			>
				...
				<div className="mb-4">
					...
					<input
						...
						type="email"
						name="email"
						className={`appearance-none border rounded w-full py-2 px-3 text-grey-darker ${email.error
							? 'border-red-500'
							: ''}`}
				...
				<div className="mb-4">
					...
					<input
						...
						id="password"
						name="password"
						className={`appearance-none border rounded w-full py-2 px-3 text-grey-darker  ${password.error
							? 'border-red-500'
							: ''}`}
						minLength={8}
						required
