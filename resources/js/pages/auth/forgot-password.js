...
function ForgotPassword() {
	...
	const handleSubmit = (e) => {
		...
		forgotPassword({ email: email.value }).then((status) => setResetFeedback(status)).catch((error) => {
			email.parseServerError(error.response.data.errors);
		});
	};
					...
					<div className="mb-4 mt-3">
						...
						<div className="mt-6 pt-6 border-t border-grey-light">
							...
							<div className="text-gray-600 text-sm pt-2">
								Be sure to check your spam filter for an email from support@email.com
							</div>
						</div>
					</div>
				</form>
